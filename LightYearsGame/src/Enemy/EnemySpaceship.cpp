...
namespace ly {
    ...
    EnemySpaceship::EnemySpaceship(World* owningWorld, const std::string& texturePath, float collisionDamage, float rewardSpawnWeight, const List<RewardFactoryFunc> rewards)
        : Spaceship{ owningWorld, texturePath }, mCollisionDamage{ collisionDamage }, mRewardFactories{ rewards }, mScoreAwardAmt{ 10 }, mRewardSpawnWeight{ rewardSpawnWeight }
    {
        SetTeamID(2);
    }
    ...
    void EnemySpaceship::SetScoreAward(unsigned int amt)
    {
        mScoreAwardAmt = amt;
    }
    ...
    void EnemySpaceship::SpawnReward()
    {
        ...
        int pick = RandomRange(0, mRewardFactories.size());
    ...
    {
        Spaceship::OnActorEndOverlap(other);
